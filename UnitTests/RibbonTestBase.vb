...
Public MustInherit Class RibbonTestBase
	...
	<TestMethod>
	Public Sub DataIsSynchronized()
		For Each element As RibbonElement In Ribbon.GetElements()
			Assert.IsTrue(OfficeHost.IsDataSynchronized(element))
		Next
	End Sub
...
#Region "Test Object Creation Methods"

	Private Function GetDummyImage() As Bitmap
		Return New Bitmap(Assembly.GetExecutingAssembly().GetManifestResourceStream("UnitTests.CloudIcon.PNG"))
	End Function

	Protected Function MakeMenu() As Menu
		Dim builder As MenuBuilder = New MenuBuilder()

		If random.NextBoolean() Then
			builder.WithLabel("My menu")
		Else
			builder.WithLabel("The Coolest menu", AddressOf GetLabel)
		End If

		If random.NextBoolean() Then
			builder.WithSuperTip("The most super menu.")
		Else
			builder.WithSuperTip("An even more amazing supertip.", AddressOf GetSuperTip)
		End If

		If random.NextBoolean() Then
			builder.WithImage(Common.DollarSign)
		Else
			builder.WithImage(GetDummyImage(), AddressOf GetImage)
		End If

		If random.NextBoolean() Then
			builder.Disabled()
		Else
			builder.Enabled(AddressOf GetEnabled)
		End If

		If random.NextBoolean() Then
			builder.WithLargeItems()
		End If

		If random.NextBoolean() Then
			builder.WithDescription("This is the description.", AddressOf GetDescription)
		End If
	...
	Protected Function MakeRibbonWithOneTabAndOneGroup(ParamArray controls As RibbonElement()) As Ribbon
		Return New RibbonBuilder().
			WithTab(New TabBuilder().WithLabel("Test Tab").
				WithGroup(New GroupBuilder().WithLabel("Test Group").
