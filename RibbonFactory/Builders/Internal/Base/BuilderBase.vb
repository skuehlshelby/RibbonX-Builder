...
Imports RibbonX.Controls.Attributes.Internal
...
Namespace Builders.Internal.Base
    ...
    Friend Class BuilderBase(Of TElement As RibbonElement)
        ...
        Public Sub New(Optional template As RibbonElement = Nothing)
            If template IsNot Nothing Then
                Dim defaultProvider As IAttributeSource = TryCast(template, IAttributeSource)
                ...
                Else
                    _attributes.OverwriteWithIntersectionOf(defaultProvider.GetAttributes())
                End If
            End If
        End Sub
...
#Region "Add Handlers"
        ...
        Private Function FromEventArgs(Of T As System.EventArgs)(actions As IEnumerable(Of Action(Of T))) As IEnumerable(Of EventHandler(Of T))
            Return actions.Select(Function(a) New EventHandler(Of T)(Sub(sender, e) a.Invoke(e)))
        End Function

        Private Function FromElementAndEventArgs(Of T As RibbonElement, K As System.EventArgs)(actions As IEnumerable(Of Action(Of T, K))) As IEnumerable(Of EventHandler(Of K))
