...
Namespace Builders
    ...
    Friend NotInheritable Class GalleryBuilder
        ...
        Private Function InsertBeforeQ(qualifiedControl As IRibbonElement) As IGalleryBuilder Implements IGalleryBuilder.InsertBefore
        ...
        Private Function InsertAfterQ(qualifiedControl As IRibbonElement) As IGalleryBuilder Implements IGalleryBuilder.InsertAfter
        ...
        Private Function HideItemImages() As IGalleryBuilder Implements IGalleryBuilder.HideItemImages
            ...
            Return Me
        End Function
        Public Function OnSelectionChange(getSelected As FromControl(Of Integer), setSelected As SelectionChanged) As IGalleryBuilder Implements IGalleryBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            Return Me
        End Function
        Public Function OnSelectionChange(getSelected As FromControl(Of String), setSelected As SelectionChanged, action As Action(Of IActionBuilder(Of IGallery, IItem))) As IGalleryBuilder Implements IGalleryBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            OnActionBase(action, NameOf(IGallery.SelectionChanging), NameOf(IGallery.SelectionChanged))
            Return Me
        End Function
        ...
        Private Function WithItemDimensions(height As Integer, width As Integer, heightCallback As FromControl(Of Integer), widthCallback As FromControl(Of Integer)) As IGalleryBuilder Implements IGalleryBuilder.WithItemDimensions
            ...
            Return Me
        End Function
        Public Function OnSelectionChange(getSelected As FromControl(Of String), setSelected As SelectionChanged) As IGalleryBuilder Implements IGalleryBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            Return Me
        End Function
        Public Function OnSelectionChange(getSelected As FromControl(Of Integer), setSelected As SelectionChanged, action As Action(Of IActionBuilder(Of IGallery, IItem))) As IGalleryBuilder Implements IGalleryBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            OnActionBase(action, NameOf(IGallery.SelectionChanging), NameOf(IGallery.SelectionChanged))
            Return Me
        End Function

        Public Function WithBlank(blank As IItem) As IGalleryBuilder Implements IGalleryBuilder.WithBlank
            Me.blank = blank
            Return Me
        End Function

        Public Function WithButtons(ParamArray buttons() As IButton) As IGalleryBuilder Implements IGalleryBuilder.WithButtons
        ...
        Public Function WithTag(tag As Object) As IGalleryBuilder Implements ITag(Of IGalleryBuilder).WithTag
            WithTagBase(tag)
            Return Me
        End Function

        Public Function FromTemplate(template As IRibbonElement) As IGalleryBuilder Implements ITemplatable(Of IGalleryBuilder).FromTemplate
            FromTemplateBase(template)
            Return Me
        End Function

        Public Overrides Function Build() As Gallery
            Return New Gallery(ControlProperties, buttons.ToArray(), blank, Tag)
        End Function

        Friend Shared Function FromAction(action As Action(Of IGalleryBuilder)) As IGallery
            Dim instance As GalleryBuilder = New GalleryBuilder()
