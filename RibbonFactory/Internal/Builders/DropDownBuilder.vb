...
Namespace InternalApi
    ...
    Friend NotInheritable Class DropDownBuilder
        ...
        Private Function GetSelectedItemIdFrom(getSelected As FromControl(Of String), setSelected As SelectionChanged) As IDropdownBuilder Implements IGetSelected(Of IDropdownBuilder).GetSelectedItemIdFrom
            SelectedBase(getSelected, setSelected)
            Return Me
        End Function

        Private Function GetSelectedItemIndexFrom(getSelected As FromControl(Of Integer), setSelected As SelectionChanged) As IDropdownBuilder Implements IGetSelected(Of IDropdownBuilder).GetSelectedItemIndexFrom
            SelectedBase(getSelected, setSelected)
            Return Me
        End Function
        ...
        Private Function InsertAfterQ(qualifiedControl As IRibbonElement) As IDropdownBuilder Implements IDropdownBuilder.InsertAfter
        ...
        Private Function InsertBeforeQ(qualifiedControl As IRibbonElement) As IDropdownBuilder Implements IDropdownBuilder.InsertBefore
        ...
        Private Function OnSelectionChange(getSelected As FromControl(Of String), setSelected As SelectionChanged) As IDropdownBuilder Implements IDropdownBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            Return Me
        End Function

        Private Function OnSelectionChange(getSelected As FromControl(Of Integer), setSelected As SelectionChanged) As IDropdownBuilder Implements IDropdownBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            Return Me
        End Function

        Private Function OnSelectionChange(getSelected As FromControl(Of Integer), setSelected As SelectionChanged, action As Action(Of IActionBuilder(Of IDropDown, IItem))) As IDropdownBuilder Implements IDropdownBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            OnActionBase(action, NameOf(IDropDown.SelectionChanging), NameOf(IDropDown.SelectionChanged))
            Return Me
        End Function

        Private Function OnSelectionChange(getSelected As FromControl(Of String), setSelected As SelectionChanged, action As Action(Of IActionBuilder(Of IDropDown, IItem))) As IDropdownBuilder Implements IDropdownBuilder.OnSelectionChange
            SelectedBase(getSelected, setSelected)
            OnActionBase(action, NameOf(IDropDown.SelectionChanging), NameOf(IDropDown.SelectionChanged))
            Return Me
        End Function

        Private Function WithTag(tag As Object) As IDropdownBuilder Implements IDropdownBuilder.WithTag
            WithTagBase(tag)
            Return Me
        End Function

        Public Function FromTemplate(template As IRibbonElement) As IDropdownBuilder Implements IDropdownBuilder.FromTemplate
            FromTemplateBase(template)
            Return Me
        End Function

        Public Function WithBlank(blank As IItem) As IDropdownBuilder Implements IDropdownBuilder.WithBlank
            Me.blank = blank
            Return Me
        End Function

        Public Function WithButtons(ParamArray buttons() As IButton) As IDropdownBuilder Implements IDropdownBuilder.WithButtons
            For Each button As IButton In buttons
                Me.buttons.Add(button)
            Next
            Return Me
        End Function

        Public Overrides Function Build() As DropDown
            Return New DropDown(ControlProperties, buttons.ToArray(), blank, Tag)
        End Function

        Public Shared Function FromAction(Optional action As Action(Of IDropdownBuilder) = Nothing) As IDropDown
            Dim instance As DropDownBuilder = New DropDownBuilder()
